#!/bin/bash
# Rofi-based project selector for launching neovim
...
selected=$(echo "$menu" | rofi -dmenu -i -p "Open Neovim in" -format s -select "~/ (Home)")
...
# Launch ghostty with nvim in the selected directory
exec ghostty --working-directory="$target_dir" -e nvim
