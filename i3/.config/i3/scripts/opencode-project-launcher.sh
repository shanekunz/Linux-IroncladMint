#!/bin/bash
# Rofi-based project selector for launching opencode
...
selected=$(echo "$menu" | rofi -dmenu -i -p "Open OpenCode in" -format s -select "~/ (Home)")
...
# Launch ghostty with opencode in the selected directory
OPENCODE_BIN="$HOME/.opencode/bin/opencode"
if [ ! -f "$OPENCODE_BIN" ]; then
    # Fallback to just opencode if not found at specific path
    OPENCODE_BIN="opencode"
fi

exec ghostty --working-directory="$target_dir" -e "$OPENCODE_BIN"
